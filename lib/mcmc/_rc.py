...
## System run controller - high level run controller in charge of : ##
#    * Decorates argument parsers in user-level scripts (mcmc_init, mcmc_run) for where to find simulation config file, what kind of logging
#    * Reading parameters/options from user-specified simulation setup and parameter files (sim.cfg)
#    * Setting up logging level for simulations
#    * Providing factory methods for creating various high-level abstractions necessary to run simulation and read/right sim output:
#        - Sim_manager (controls high-level simulation operations)
#        - System (abstraction of simulated system's current state)
#        - DataManager (responsible for reading/writing sim data to, e.g. energy files or log files or whatnot)
class SysCtl:
    ...
    ## INITIALIZE RC state/params from config files (sim.cfg and params.cfg) ##
    ...
    ### LOGGING SETUP ###
    # Yikes. All this does is set up and control the logging system for simulation runs - ripped from WESTPA
